...
description: "Domain Name System (DNS) Protocol & Resolution: Recursive vs authoritative name servers, root hint zones, DNS record types (A, AAAA, CNAME, MX, TXT, SRV), DNSSEC signing, and DNS over HTTPS/TLS (DoH/DoT)."
...
# Domain Name System (DNS) Protocol & Resolution

Recursive vs authoritative name servers, root hint zones, DNS record types (A, AAAA, CNAME, MX, TXT, SRV), DNSSEC signing, and DNS over HTTPS/TLS (DoH/DoT).

## Standard Operating Procedure & Methodology

### 1. Conceptual & Theoretical Foundations
Formulate the theoretical principles governing domain name system (dns) protocol & resolution:
