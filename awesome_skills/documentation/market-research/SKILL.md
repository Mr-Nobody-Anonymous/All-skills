---
name: market-research
description: |
  Conduct structured market research with frameworks and actionable insights.
  TRIGGERS - Use when user wants market research, market analysis, TAM/SAM/SOM, or industry overview.
source: "https://github.com/Winbda/claude-skills-collection"
source_repository: "Winbda/claude-skills-collection"
source_path: "skills/market-research/SKILL.md"
license: "MIT"
imported_at: "2026-09-20"
---

# Market Research

## Output Format

```markdown
# Market Research: [Topic/Industry]

## Executive Summary
[3-5 sentences covering market size, growth, key trends, and opportunity]

## Market Size
- **TAM** (Total Addressable Market): $[X]B
- **SAM** (Serviceable Addressable Market): $[X]M
- **SOM** (Serviceable Obtainable Market): $[X]M
- **Growth rate**: [X]% CAGR

## Industry Trends
1. **[Trend 1]**: [description and impact]
2. **[Trend 2]**: [description and impact]
3. **[Trend 3]**: [description and impact]

## Target Customer Profile
- **Demographics**: [who they are]
- **Pain points**: [what they struggle with]
- **Buying behavior**: [how they purchase]
- **Budget**: [what they spend]

## Competitive Landscape
| Player | Market Share | Positioning | Strength | Weakness |
|--------|------------|-------------|----------|----------|
| [Co] | [X%] | [position] | [strength] | [weakness] |

## Opportunities
1. [Underserved segment]
2. [Emerging trend to capitalize on]
3. [Competitor weakness to exploit]

## Threats
1. [Market risk]
2. [Competitive risk]
3. [Regulatory risk]

## Recommendations
1. [Strategic recommendation]
2. [Tactical recommendation]
3. [Timing recommendation]

## Sources
[List of sources used]
```
