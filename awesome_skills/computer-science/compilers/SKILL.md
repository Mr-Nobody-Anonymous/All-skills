---
name: compilers
description: "Compiler Architecture & Intermediate Representation: Lexical analysis (lex/flex), LL(k) and LR(k) parsing (Yacc/Bison), Abstract Syntax Trees (AST), SSA form (Static Single Assignment), LLVM IR, and register allocation (graph coloring)."
...
Lexical analysis (lex/flex), LL(k) and LR(k) parsing (Yacc/Bison), Abstract Syntax Trees (AST), SSA form (Static Single Assignment), LLVM IR, and register allocation (graph coloring).

## Standard Operating Procedure & Methodology

### 1. Conceptual & Theoretical Foundations
Formulate the theoretical principles governing compiler architecture & intermediate representation:
