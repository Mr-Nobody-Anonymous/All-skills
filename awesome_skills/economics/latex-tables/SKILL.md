---
name: latex-tables
description: "> Generates publication-ready LaTeX tables produced by code (Stata `esttab`/`outreg2`, R `modelsummary`/`fixest::etable`, Python `stargazer`/`pyfixest.etable`) and writes them directly to the paper's `tabs/` folder. Defaults to DIME's 'full replicability' standard (no copy-paste from any console), `"
category: economics
version: 1.0.0
...
source_path: "_skills/writing/latex-tables/SKILL.md"
license: "MIT"
