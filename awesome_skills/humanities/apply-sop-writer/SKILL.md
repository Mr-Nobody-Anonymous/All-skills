...
source_path: "skills/academic-career/apply-sop-writer/SKILL.md"
license: "MIT"
...
**Check the "why this programme" material next.** A fit paragraph that only echoes the programme's own brochure is evidence of nothing.

- **Brochure-echo anti-pattern.** Never repeat a programme's selling points back to it ("your world-class faculty", "your unique interdisciplinary environment", "your strong industry links"). The committee wrote that copy. Same swap test as above: if the sentence survives moving to a competing programme, cut it.
- **Build from verified differentiators and outcomes.** Use a programme profile from `apply-program-theme-extractor` (pass C) where one exists; otherwise search the web for the programme's learning outcomes, graduate profile, and what it offers, recording URL and access date. Extract that profile **first and applicant-free** — its content must be unchanged for a different applicant, so do not let the applicant's goal, draft, or background decide what is read, kept, or phrased — and only then relate it to the applicant. Use only differentiators backed by a page — a named partner, a mobility structure, a compulsory component, a facility — and never an unbacked claim.
- **Tie each one to a need the applicant has.** "Your compulsory semester at [partner] gives me [specific capability my next question needs]" does work; naming the differentiator alone does not.
- **Match "what after" to the intended graduate.** Where the programme declares whom it aims to produce, the post-degree plan should be legible against that profile — without changing the applicant's real goal. Facts fixed, weights vary: where the goal and the profile genuinely diverge, flag it and route to `apply-program-fit-mapper` rather than rewording the goal.
...
## Program-fit issues
<!-- include brochure-echo sentences and differentiators with no backing page -->
