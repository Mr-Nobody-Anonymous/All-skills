...
Ship `scripts/compile.sh` alongside the `.tex` so the deck builds in one step with no remembered flags:

```bash
./scripts/compile.sh talk.tex        # latexmk if present, else two pdflatex passes
./scripts/compile.sh -c talk.tex     # compile, then clean aux files
ENGINE=lualatex ./scripts/compile.sh talk.tex
```
