...
source_path: "skills/academic-career/apply-profile-reader/SKILL.md"
license: "MIT"
...
- The user asks for a reusable profile before using `apply-sop-writer`, `apply-cv-builder`, `apply-program-fit-mapper`, `apply-cold-email-drafter`, `apply-recommendation-letter-strategist`, or `apply-package-auditor`.
...
- The user wants a final SOP draft or essay revision. Use `apply-sop-writer` after extracting the profile.
- The user wants a finished academic CV. Use `apply-cv-builder` after extracting the evidence inventory.
- The user wants faculty matching from scratch. Use `apply-program-fit-mapper` after extracting research interests and constraints.
- The user wants a full cross-document readiness audit. Use `apply-package-auditor` after extracting the profile.
...
## Downstream use
- For `apply-sop-writer`: [themes and experiences to emphasize]
- For `apply-cv-builder`: [CV sections/items to strengthen]
- For `apply-program-fit-mapper`: [interests, constraints, faculty-fit clues]
- For `apply-cold-email-drafter`: [compact pitch material]
- For `apply-recommendation-letter-strategist`: [who can validate what]
- For `apply-package-auditor`: [cross-document risks]
