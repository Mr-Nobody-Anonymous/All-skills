...
source_path: "skills/academic-career/apply-interview-prep/SKILL.md"
license: "MIT"
...
2. **Why this programme, this host, this group.** Must contain something that could not be said about any other programme. Generic answers here are the most commonly cited weakness. Build it from the programme profile of `apply-program-theme-extractor` (pass C): differentiators that passed its swap test and have a backing page, the declared learning outcomes, and the intended graduate — never the brochure's own selling points repeated back to the panel. Where no profile exists, fetch the programme's learning-outcomes, graduate-profile, and offer pages live, recording URL and access date, and keep only claims a page backs. Either way, extract the profile **first and applicant-free** — its content must be unchanged for a different applicant, so do not let the applicant's goal, draft, or background decide what is read, kept, or phrased — and only then tie each differentiator to something the user specifically needs.
3. **Why this degree, and what after.** Phrase it for the actual level. For a doctoral application: why a PhD, why now, and what research career follows. For a master's application: why this specific programme, why it is the right next step from your background, and what you will do with it — for master's scholarships with a development, bilateral, or leadership mission, "what after" is a **scored criterion**, not a closing pleasantry. Assessor types A and C weight this heavily in both cases. You are expected to have thought that far ahead. Where the programme declares the graduate it intends to produce, make the answer legible against that profile without changing the user's real plan; where the two genuinely diverge, surface it before the interview rather than improvising around it.
