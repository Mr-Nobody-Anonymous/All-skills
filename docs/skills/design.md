# Design Skills

_Generated: 2026-09-02T04:38:48+00:00_

Skills for design work — UI/UX, frontend design, presentations, and branding. These produce prose wireframes, design feedback, narrative arcs for talks, and naming/voice guidance.

**4 skills in this category.**

## Skills

### `design.branding`

Brand identity work — voice, naming, visual language, and consistency.

- **Risk:** low
- **Path:** `design/branding`
- **Aliases:** `brand`, `identity`, `voice`, `naming`
- **Triggers:**
  - brand this
  - help with brand voice
  - naming ideas
  - visual identity
- **Source:** custom
- **Version:** 1.0.0

Help the user develop and apply brand identity: voice, naming, visual language, and consistency across touchpoints.

### `design.frontend-design`

Design and implement frontend interfaces — HTML, CSS, layouts, and visual design for the web.

- **Risk:** low
- **Path:** `design/frontend-design`
- **Aliases:** `html-css`, `web-design`, `landing-page`
- **Triggers:**
  - design this landing page
  - style this page
  - frontend design
  - layout this
  - CSS for this
- **Source:** custom
- **Version:** 1.0.0

Design and implement frontend pages — semantic HTML, accessible CSS, responsive layouts, and component composition.

### `design.presentations`

Design and structure presentations — narrative arc, slide content, visual hierarchy, and delivery notes.

- **Risk:** low
- **Path:** `design/presentations`
- **Aliases:** `slides`, `deck-design`, `presentation-design`
- **Triggers:**
  - design a presentation
  - structure my talk
  - slides for
  - presentation outline
- **Source:** custom
- **Version:** 1.0.0

Help the user design and structure a presentation. Outline the narrative arc, draft slide content, and note visual / delivery considerations.

### `design.ui-ux`

Design UI / UX — flows, screens, components, and interaction patterns. Produce wireframes-in-prose and design feedback.

- **Risk:** low
- **Path:** `design/ui-ux`
- **Aliases:** `ux`, `ui`, `interaction-design`, `wireframe`
- **Triggers:**
  - design this UI
  - UX feedback
  - how should this screen work
  - wireframe this
- **Source:** custom
- **Version:** 1.0.0

Help the user design a screen, flow, or interaction. Produce wireframes-in-prose, identify states, and give actionable design feedback.

