# Documents Skills

_Generated: 2026-09-02T13:11:07+00:00_

Skills for working with common document formats: PDF, DOCX, XLSX, PPTX, Markdown, and CSV. They focus on reliable extraction, structured generation, and content-aware summarization rather than format-perfect output.

**6 skills in this category.**

## Skills

### `documents.csv`

Read, parse, write, and clean CSV files — dialect handling, type inference, and validation.

- **Risk:** low
- **Path:** `documents/csv`
- **Aliases:** `csv-reading`, `csv-cleaning`, `csv-generation`
- **Triggers:**
  - read this CSV
  - parse CSV
  - clean this CSV
  - generate a CSV
- **Source:** custom
- **Version:** 1.0.0

Work with CSV files: parse with the right dialect, validate, clean, and write.

### `documents.docx`

Read, edit, and generate Microsoft Word .docx files — extract text, modify structure, and produce documents programmatically.

- **Risk:** low
- **Path:** `documents/docx`
- **Aliases:** `word`, `docx-reading`, `docx-generation`
- **Triggers:**
  - read this Word doc
  - extract from docx
  - generate a Word document
  - edit this docx
- **Source:** custom
- **Version:** 1.0.0

Work with .docx files: read content, edit structure, generate documents from templates.

### `documents.markdown`

Read, write, lint, and transform Markdown — headings, links, code fences, tables, and common dialects.

- **Risk:** low
- **Path:** `documents/markdown`
- **Aliases:** `md`, `markdown-lint`, `gfm`
- **Triggers:**
  - format this markdown
  - lint markdown
  - convert markdown to
  - markdown style
- **Source:** custom
- **Version:** 1.0.0

Work with Markdown — write clean, consistent MD; convert to / from other formats; lint for common issues.

### `documents.pdf`

Read, extract text, summarize, and produce PDF files. Parse structured content where possible.

- **Risk:** low
- **Path:** `documents/pdf`
- **Aliases:** `pdf-reading`, `pdf-generation`, `pdf-extraction`
- **Triggers:**
  - read this PDF
  - extract text from PDF
  - summarize this PDF
  - generate a PDF
  - make a PDF
- **Source:** custom
- **Version:** 1.0.0

Work with PDF files — read content, extract text and tables, summarize, and produce PDFs.

### `documents.pptx`

Read, edit, and generate PowerPoint .pptx files — extract slides, build decks from outlines.

- **Risk:** low
- **Path:** `documents/pptx`
- **Aliases:** `powerpoint`, `slides`, `deck`, `presentation-generation`
- **Triggers:**
  - read this PowerPoint
  - generate a slide deck
  - make a presentation
  - extract slides from
- **Source:** custom
- **Version:** 1.0.0

Work with PowerPoint files: read existing decks, generate decks from outlines, edit slides.

### `documents.xlsx`

Read, edit, and generate Excel .xlsx files — read sheets, compute, and produce workbooks programmatically.

- **Risk:** low
- **Path:** `documents/xlsx`
- **Aliases:** `excel`, `spreadsheet`, `xlsx-reading`
- **Triggers:**
  - read this spreadsheet
  - extract from xlsx
  - generate an Excel file
  - edit this spreadsheet
- **Source:** custom
- **Version:** 1.0.0

Work with .xlsx files: read sheets and cells, compute with the data, and produce workbooks.

